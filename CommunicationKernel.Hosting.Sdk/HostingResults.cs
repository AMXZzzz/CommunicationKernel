...
// -----------------------------------------------------------------------------
// 文件: HostResults.cs
// 层级: 客户端层 — Hosting.Sdk 对外结果契约
// 作用: 定义 Hosting.Sdk 全部方法的返回形态。
//
// 为什么单独成文件、且一律用具名 record：
//   Hosting.Sdk 是要发 NuGet 的对外契约。此前 RegisterRouteAsync / RemoveRouteAsync /
...
//
// 统一的失败形态:
//   所有「可能失败的操作」共享 HostingOperationResult 的三字段形状
...
namespace CommunicationKernel.Hosting.Sdk
{
    ...
    /// <summary>
    /// Hosting.Sdk 所有可失败操作的公共结果形状。
    /// </summary>
    /// <remarks>
    /// <para>
    /// 本类型及其派生类型一律以「返回结果」而非「抛异常」表达业务失败：
    /// 传输异常在 <see cref="HostingClient"/> 内部就被转换成
    ...
    public record HostingOperationResult(bool Success, string ErrorCode, string ErrorMessage)
    {
        /// <summary>构造一个成功结果，错误字段置空。</summary>
        public static HostingOperationResult Ok() =>
        ...
        public static HostingOperationResult Fail(string code, string message) =>
    ...
    /// </summary>
    /// <remarks>
    /// 这里不复用 <see cref="HostingOperationResult"/>：健康检查探测的是「宿主是否可达」，
