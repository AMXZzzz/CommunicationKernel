// -----------------------------------------------------------------------------
// 文件: Services/InProcessHostingClient.cs
// 层级: UI 层 — WebMaster 组合根内侧
// 作用: IHostingClient 的进程内实现。UI 仍然只看见 route_id 与 DTO，
...
/// <summary>把 EngineRuntime 适配成 UI 已经在用的 <see cref="IHostingClient"/>。</summary>
internal sealed class InProcessHostingClient : IHostingClient
{
    ...
    public InProcessHostingClient(EngineRuntime engine, IRouteAssemblyService assembly)
    {
        _engine = engine;
        _assembly = assembly;
    }
