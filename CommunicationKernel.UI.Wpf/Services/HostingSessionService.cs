...
// -----------------------------------------------------------------------------
// 文件: Services/HostingSessionService.cs
// 层级: UI 层 — WPF 服务
// 作用: 维护与 Hosting.App 的会话状态（在线与否、版本、路由数），后台周期性健康探测。
// 调用链:
//   App 启动 → HostingSessionService.Start() → HostingClient.HealthAsync() → gRPC
...
//   Web 端的对应实现是 UI.WebMaster/Services/EngineSession.cs，两端职责划分现已一致。
//
// 方向纪律:
...
{
    /// <summary>
    /// Hosting.App 会话状态，单例。周期性健康探测并在状态变化时发出 <see cref="Changed"/>。
    /// </summary>
    public sealed class HostingSessionService : IDisposable
    {
        // ====================================================================
        // 常量
        // ====================================================================
        ...
        /// <param name="client">Hosting.App 客户端，必填。</param>
        /// <param name="log">可选日志器，为 null 时静默。</param>
        public HostingSessionService(IHostingClient client, IAppLogger log = null)
        {
        ...
        /// <summary>Hosting.App 是否在线。初始为 false，首次探测成功后置 true。</summary>
