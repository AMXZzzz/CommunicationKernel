...
// 层级: UI 层 — WPF 服务
// 作用: 维护与 Host.App 的会话状态（在线与否、版本、路由数），后台周期性健康探测。
// 调用链:
...
//   Web 端的对应实现是 UI.Web/Services/HostSession.cs，两端职责划分现已一致。
//
// 方向纪律:
...
{
    /// <summary>
    /// Host.App 会话状态，单例。周期性健康探测并在状态变化时发出 <see cref="Changed"/>。
    /// </summary>
    public sealed class HostSessionService : IDisposable
    {
        ...
        /// <param name="client">Host.App 客户端，必填。</param>
        ...
        /// <summary>Host.App 是否在线。初始为 false，首次探测成功后置 true。</summary>
