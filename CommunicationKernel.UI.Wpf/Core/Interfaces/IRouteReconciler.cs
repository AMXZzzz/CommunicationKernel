...
// 层级: UI 层 — WPF 核心接口
// 作用: 按需把本地设备配置重新推给 EngineHost.App，使丢失的内存路由恢复可用。
// -----------------------------------------------------------------------------
...
{
    /// <summary>
    /// 路由对账器：确保某条路由在 EngineHost.App 侧确实存在。
    /// </summary>
    /// <remarks>
    /// <para>
    /// EngineHost.App 的路由是纯内存对象，进程重启即全部丢失。此时上位机侧
    ...
    {
        /// <summary>
        /// 确保指定路由在 EngineHost.App 侧存在，必要时用本地留存的配置重新注册。
        /// </summary>
