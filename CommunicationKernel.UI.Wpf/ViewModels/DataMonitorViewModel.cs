...
public sealed class DataMonitorViewModel : ViewModelBase {
    ...
    /// <summary>手动刷新命令：触发 IDeviceService.Load() 重新从 EngineHost.App 拉取路由列表。</summary>
    ...
    private void RebuildMonitoredDevices() {
        // 获取 Dispatcher：单元测试等无 Application 时直接同步执行
        var dispatcher = Application.Current?.Dispatcher;
        if (dispatcher == null || dispatcher.CheckAccess()) {
            // 已在 UI 线程或无 Dispatcher，直接执行
            DoRebuild();
        } else {
            // 从后台线程切回 UI 线程再改 ObservableCollection
