...
namespace CommunicationKernel.Engine.Models;

/// <summary>注册一条路由所需的全部参数。</summary>
/// <remarks>
/// 本类型此前是 <c>HostRuntime</c> 的嵌套类型，造成两个问题：
/// SDK 消费者要写 <c>new HostRuntime.RegisterRouteCommand { … }</c>；
/// 且 <c>IRouteAssemblyService</c> 的参数类型指向上层具体类，
/// 任何人想另行实现该抽象都被迫依赖 <c>HostRuntime</c>。
