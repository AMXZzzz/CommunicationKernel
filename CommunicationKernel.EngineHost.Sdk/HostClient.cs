...
// 调用链:
//   UI（WPF / Blazor / 其他）→ HostClient → gRPC Channel → EngineHostingServiceApp
// -----------------------------------------------------------------------------
...
public sealed class HostClient : IHostClient {
    ...
    public HostClient(string serverAddress, ILogger<HostClient> logger = null) {
        // 记录日志器，null 时退化为 NullLogger 保证无条件安全调用
        _logger = logger ?? Microsoft.Extensions.Logging.Abstractions.NullLogger<HostClient>.Instance;
    ...
    /// <summary>
    /// 向 EngineHostingServiceApp 注销路由，停止对应的 PLC 连接。
    ...
    public async Task<RemoveRouteResultDto>
        ...
        try {
            ...
            return new RemoveRouteResultDto(false, "UNIMPLEMENTED",
                "服务端不支持删除路由，请升级 EngineHostingServiceApp 后重试");
        }
        catch (RpcException ex) {
    ...
    /// <summary>
    /// 查询 EngineHostingServiceApp 已加载的协议插件描述符列表。
