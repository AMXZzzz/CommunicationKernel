...
{
    // 独立分类名，journalctl 可按 Hosting.App.Endpoint 过滤
    var logger = app.Services.GetRequiredService<ILoggerFactory>().CreateLogger("Hosting.App.Endpoint");
...
catch (Exception ex) when (IsAddressInUse(ex))
{
    ...
    ReportStartupFailure(
        "监听地址已被占用，宿主无法启动。\n\n" +
        "多半是已经有一个 Hosting.App 在跑了（任务管理器里看 CommunicationKernel.Hosting.App）。\n" +
