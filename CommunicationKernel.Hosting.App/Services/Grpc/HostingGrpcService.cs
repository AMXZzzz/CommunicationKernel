...
using CommunicationKernel.Hosting.Grpc.V1;
...
// -----------------------------------------------------------------------------
// 文件: HostingGrpcService.cs
// 层级: Hosting.App / Services / Grpc
// 作用: Hosting.App gRPC 对外服务实现（协议无关请求模型）。
// 说明:
...
namespace CommunicationKernel.Hosting.App.Services;
...
/// -----------------------------------------------------------------------------
/// </summary>
public sealed class HostingGrpcService : HostingApi.HostingApiBase {
    // 服务版本常量集中定义，避免多处硬编码。
    ...
    private readonly ILogger<HostingGrpcService> _logger;

    // 注入内核、插件装配与日志；三者均为单例，与宿主同寿
    public HostingGrpcService (
        // 路由注册 / 读写 / 状态订阅的唯一入口
        EngineRuntime hostRuntime,
        // 协议插件清单与宿主机器串口枚举
        IRouteAssemblyService routeAssemblyService,
        // 推流丢事件、注册失败等诊断日志
        ILogger<HostingGrpcService> logger) {
